...
def sacsma(forcings:  npt.NDArray[np.floating], par: npt.ArrayLike, initstate: npt.ArrayLike = [0,0,500,500,500,0]) ->  npt.NDArray[np.float64]:
    ...
    ninc = np.int(np.floor(1.0 + 0.2*(uzfwc+twx)))  # Number of time increments that interval is divided into for further soil-moisture accountng
    ...
    outflows = np.hstack([simflow, surf_tot, base_tot])
    outstate = np.hstack([uztwc, uzfwc, lztwc, lzfsc, lzfpc, adimc])

    return outflows, outstate